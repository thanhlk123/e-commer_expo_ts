...
import goodsItemData from "@demoData/GoodsItems";
...
const GoodsItem = (item) => {
  return (
    <View>
      ...
      >
        <Image source={item.img} style={styles.imgItem} resizeMode="contain" />
        <View>
          <Text>{item.name}</Text>
          <AntDesign />
        </View>
...
type ScreenMainProps = {
  ...
  sortList: { id: number; topic: string }[];
  indexActive: number;
    ...
    <View style={styles.modalContent}>
      ...
      {this.state.sortList.map((item, index) => (
        <TouchableOpacity
          ...
          onPress={(index) => {
            this.setState({ visibleModal: false, indexActive: item.id });
          }}
        >
          <Text
            style={[
              styles.sortText,
              this.state.indexActive == index ? { color: "blue" } : {},
            ]}
          >
            {item.topic}
          </Text>
    ...
    return (
      <View style={styles.container}>
        <View style={[styles.row, styles.filterArea]}>
          ...
          <TouchableOpacity style={[styles.row, styles.refineButton]}>
            <Text />
            <AntDesign name="down" size={18} color="black" />
            <Text>REFINE</Text>
            <Text />
